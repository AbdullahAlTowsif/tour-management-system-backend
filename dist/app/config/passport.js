    ...
    try {
        ...
        if (isGoogleAuthenticated && !isUserExist.password) {
            return done(null, false, {
                message: "You have authenticated via Google. If you want to login with credentials, then at first login with goolge and set a password for your Gmail and then you can login with email and password",
            });
        }
...
passport_1.default.use(new passport_google_oauth20_1.Strategy({
    ...
    try {
        ...
        if (isUserExist &&
            (isUserExist.isActive === user_interface_1.IsActive.BLOCKED || isUserExist.isActive === user_interface_1.IsActive.INACTIVE)) {
            // throw new AppError(
            //   httpStatus.BAD_REQUEST,
            //   `User is ${isUserExist.isActive}`
            // );
